-- Migration: Adicionar campo acumulativo_desde_inicio
-- Permite diferenciar entre acumulativo por frequência e acumulativo desde o início

ALTER TABLE public.regras_atividades 
ADD COLUMN IF NOT EXISTS acumulativo_desde_inicio BOOLEAN DEFAULT FALSE;

COMMENT ON COLUMN public.regras_atividades.acumulativo_desde_inicio IS 
'Quando true, o acumulativo sempre começa do módulo inicial. Quando false, usa o intervalo baseado na frequência.';

-- Atualizar função para suportar ambos os comportamentos
            ...
            IF v_contador >= r_regra.comecar_no_modulo THEN
                IF ((v_contador - r_regra.comecar_no_modulo) % r_regra.frequencia_modulos = 0) THEN
                    IF r_regra.acumulativo THEN
                        -- Se acumulativo_desde_inicio = true, sempre começa do módulo inicial
