...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    const period = (searchParams.get("period") || "anual") as
      | "semanal"
      | "mensal"
      | "anual";
    ...
    const heatmap = await dashboardAnalyticsService.getHeatmapData(
      targetUserId,
      client,
      period,
      empresaId,
    ...
    console.error("Dashboard Heatmap API Error:", error);
    return NextResponse.json(
      {
        error:
