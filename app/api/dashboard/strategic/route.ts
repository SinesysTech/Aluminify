...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    const strategic = await dashboardAnalyticsService.getStrategicDomain(
      targetUserId,
      client,
      period,
      empresaId,
    ...
    return NextResponse.json({ data: strategic });
  } catch (error) {
    console.error("Dashboard Strategic API Error:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Erro ao carregar domínio estratégico",
      },
      { status: 500 },
    );
  }
