...
The 'aluno' module must be self-contained within `app/[tenant]/(dashboard)/aluno`.

#### Scenario: Directory Structure
Given the project root
When I list `app/[tenant]/(dashboard)/aluno`
...
Given a file inside `app/[tenant]/(dashboard)/aluno`
Then it should not import from `components/aluno` (legacy)
And it should not import from `app/(dashboard)/aluno` (legacy).
...
Then they should be located in `app/[tenant]/(dashboard)/aluno/components/dashboard`.
...
Then `app/(dashboard)/aluno` should not exist.
