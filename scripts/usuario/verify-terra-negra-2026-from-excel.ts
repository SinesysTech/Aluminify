/**
 * Verifica se todos os alunos da planilha Terra Negra (alunos.xlsx)
 * estão cadastrados e matriculados no curso Terra Negra 2026.
 *
 * Uso:
 *   npx tsx scripts/usuario/verify-terra-negra-2026-from-excel.ts
 *   npx tsx scripts/usuario/verify-terra-negra-2026-from-excel.ts "c:\path\to\alunos.xlsx"
 *
...
async function main() {
  ...
  console.log(
    "=== Verificação: Alunos da planilha vs curso Terra Negra 2026 ===\n",
  );
  ...
  const empresaPick = (empresa ?? []).find(
    (e) => String(e.nome ?? "").trim().toLowerCase() === "terra negra",
  ) ?? (empresa ?? [])[0];
  if (errEmpresa || !empresaPick?.id) {
    console.error("\n❌ Empresa 'Terra Negra' não encontrada no banco.");
    process.exit(1);
  }
  ...
  const { data: curso, error: errCurso } = await supabase
    .from("cursos")
    .select("id, nome")
    .eq("empresa_id", empresaPick.id)
    .eq("nome", "Terra Negra 2026")
    .maybeSingle();
  if (errCurso || !curso?.id) {
    console.error("\n❌ Curso 'Terra Negra 2026' não encontrado na empresa.");
    process.exit(1);
  }
  ...
  for (let i = 0; i < emailsUnicos.length; i += BATCH) {
    const chunk = emailsUnicos.slice(i, i + BATCH);
  ...
  console.log("\n--- Resultado ---");
  console.log("Empresa:", empresaPick.nome);
  ...
  const todosFaltantes = [...naoCadastrados, ...cadastradosNaoMatriculados];
  console.log("\n--- E-mails NÃO listados/corretos no curso (para você) ---");
  if (todosFaltantes.length === 0) {
    console.log("Nenhum. Todos os alunos da planilha estão cadastrados e matriculados no Terra Negra 2026.");
  } else {
    todosFaltantes.forEach((e) => console.log(e));
  }
  ...
  if (todosOk) {
    console.log(
      "Todos os alunos da planilha estão cadastrados e matriculados no curso Terra Negra 2026.",
    );
  } else {
    ...
    if (cadastradosNaoMatriculados.length)
      console.log(
        `  - ${cadastradosNaoMatriculados.length} usuário(s) cadastrado(s) mas não matriculados no curso Terra Negra 2026.`,
      );
  }
  console.log("");
}
