...
async function main() {
  ...
  const _cursoIds = (cursos ?? []).map((c: { id: string }) => c.id);
  ...
  const _staffIds = new Set((staffRows ?? []).map((r) => r.id));
  ...
  const { data: acRows } = await supabase
    .from("alunos_cursos")
    .select("usuario_id");
  (acRows ?? []).forEach((r: { usuario_id?: string | null }) => {
  ...
  try {
    ...
    if (Array.isArray(admins))
      admins.forEach((a: { user_id?: string }) => {
        if (a?.user_id) adminUserIds.add(a.user_id);
      });
  } catch {
  ...
  const papeisMap = new Map(
    (papeisData ?? []).map((p: { id: string; tipo: string }) => [p.id, p.tipo]),
  );

  // Saída
  console.log(
    "--- Terra Negra: usuários com vínculo duplo (professor/admin + aluno) ---\n",
  );
  ...
  console.log(
    "  " +
      "E-mail".padEnd(45) +
      "  Nome (resumo)                    Papel staff    Admin",
  );
  console.log(
    "  " + "-".repeat(45) + "  " + "-".repeat(32) + "  " + "-".repeat(8),
  );
