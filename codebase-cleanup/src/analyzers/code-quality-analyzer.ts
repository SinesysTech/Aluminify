...
export class CodeQualityAnalyzer extends BasePatternAnalyzer {
  ...
  private allFunctions: Map<string, { file: string; node: Node; signature: string }[]> = new Map();
  private allConstants: Map<string, { file: string; node: Node; value: string }[]> = new Map();
  ...
  private detectDuplicateFunctions(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    for (const func of functions) {
      ...
      for (const existingFunc of existing) {
        const similarity = this.calculateSimilarity(normalizedBody, this.normalizeCode(this.getFunctionBody(existingFunc.node) || ''));
        
        if (similarity > 0.85) { // 85% similar
          const funcName = this.getNodeName(func) || 'anonymous function';
          const existingName = this.getNodeName(existingFunc.node) || 'anonymous function';
          ...
          issues.push(
            this.createIssue({
              ...
              recommendation: `Consolidate duplicate functions by:\n` +
              ...
              estimatedEffort: 'small',
              tags: ['code-duplication', 'duplicate-function', 'maintainability'],
            })
          );
        }
      }
  ...
  private detectSimilarCodeBlocks(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    for (const block of blocks) {
      ...
      for (const otherBlock of blocks) {
        ...
        const similarity = this.calculateSimilarity(normalizedBlock, normalizedOther);

        if (similarity > 0.80 && similarity < 1.0) { // 80-99% similar (not identical)
          ...
          if (blockLocation.startLine < otherLocation.startLine) {
            issues.push(
              this.createIssue({
                ...
                recommendation: `Refactor similar code blocks by:\n` +
  ...
  private detectDuplicateConstants(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    for (const variable of variables) {
      ...
      const initializer = (variable as any).getInitializer?.();
        ...
        if (existingConst.file !== file.relativePath) {
          issues.push(
            this.createIssue({
              ...
              recommendation: `Consolidate duplicate constants by:\n` +
              ...
              estimatedEffort: 'trivial',
              tags: ['code-duplication', 'duplicate-constant', 'maintainability'],
            })
          );
        }
      }
   ...
   */
  private detectSingleLetterVariables(file: FileInfo, ast: SourceFile): Issue[] {
    const issues: Issue[] = [];
    ...
    for (const func of functions) {
      ...
      for (const param of params) {
        ...
        if (paramName.length === 1) {
          ...
          const isShortArrowFunction = func.getKind() === SyntaxKind.ArrowFunction && 
          ...
          if (!isShortArrowFunction) {
            issues.push(
              this.createIssue({
                ...
                recommendation: `Use descriptive parameter names:\n` +
  ...
  private detectInconsistentNaming(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    const namingStyles: Map<string, { style: 'camelCase' | 'snake_case' | 'PascalCase' | 'UPPER_CASE' | 'mixed', nodes: Node[] }> = new Map();
    ...
    for (const variable of variables) {
      ...
      if (!namingStyles.has(style)) {
        namingStyles.set(style, { style: style as any, nodes: [] });
      }
    ...
    const variableStyles = Array.from(namingStyles.keys()).filter(s => s !== 'PascalCase' && s !== 'UPPER_CASE');
  ...
  private detectUnclearNames(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    const vagueNames = new Set([
      'data', 'info', 'item', 'obj', 'temp', 'tmp', 'val', 'value',
      'result', 'res', 'ret', 'output', 'input', 'thing', 'stuff',
      ...
      if (this.hasUnclearAbbreviation(varName)) {
        issues.push(
          this.createIssue({
            ...
            recommendation: `Use full words instead of abbreviations:\n` +
      ...
      if (vagueNames.has(funcName.toLowerCase())) {
        issues.push(
          this.createIssue({
            ...
            recommendation: `Use descriptive function names:\n` +
  ...
  private detectCommentedOutCode(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    let commentedCodeBlock: { start: number; end: number; lines: string[] } | null = null;
  ...
  private detectUnusedExports(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    exportedDeclarations.forEach((declarations, name) => {
      for (const declaration of declarations) {
        ...
        const isUtilityFile = file.relativePath.includes('/util') || 
  ...
  private getFunctionSignature(func: Node): string {
    ...
    const params = (func as any).getParameters?.() || [];
    const paramNames = params.map((p: any) => p.getName?.() || '').join(',');
  ...
  private isConstantVariable(variable: Node): boolean {
    ...
    if (grandParent && grandParent.getKind() === SyntaxKind.VariableDeclarationList) {
      const flags = (grandParent as any).getDeclarationKind?.();
      if (flags === 'const') {
        return true;
      }
    }
